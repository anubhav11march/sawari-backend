package com.tzs.marshall.author.controller.prelogin;
...
import com.tzs.marshall.author.mailsender.EmailBean;
import com.tzs.marshall.author.mailsender.EmailService;
import com.tzs.marshall.author.service.AuthorPreLoginService;
import com.tzs.marshall.author.service.AuthorRegistrationService;
...
public class PreLoginRestController {
    ...
    public PersistentAuthorDetails userRegistration(@RequestBody PersistentAuthorDetails authorDetails, HttpServletRequest request) {
...
//        String url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort();
        ...
        authorDetails.setRoleName(Constants.ROLE_AUTHOR);
...
//        String url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort();
    ...
    String forgotPassword(@RequestBody NewsLetterEmailSubs newsLetterEmailSubs,
        ...
        String token;
        try {
            PersistentAuthorDetails persistentAuthorDetails = authorPreLoginService.handleFetchedValidUser(newsLetterEmailSubs.getEmail());
            log.info("user found!");
            log.info("Email: " + persistentAuthorDetails.getEmail());
            String url = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort();
    ...
    String resetPassword(@RequestParam Map<String, String> allRequestParams,
                               @RequestBody PersistentAuthorDetails persistentAuthorDetails, HttpSession session) {
        log.info("Confirming Token...");
        String token = allRequestParams.get("token");
        String reqType = allRequestParams.get("reqType");
        String password = persistentAuthorDetails.getPassword();
        log.info("Token: " + token + " & reqType: " + reqType);
        String flag;
        try {
    ...
    public Object getProperties(@RequestParam("property") String property) {
        return AESHProperties.getAESHProperty(property);
    }
