...
import com.ether.author.filesystem.FileBean;
import com.ether.author.filesystem.FileService;
...
@RestController
@RequestMapping(value = {"/author/content", "/admin/content"})
public class FileController {
    ...
    public ModelAndView uploadFile(@RequestParam("file") MultipartFile file, @ModelAttribute FileBean fileBean,
                                   @AuthenticationPrincipal PersistentAuthorDetails authorDetails,
                                   HttpSession session) {
        ...
        try {
            ...
            if (!Constants.ROLE_ADMIN.equals(authorDetails.getRoleName())) {
    ...
    public List<FileBean> getAllFileByUserId(@AuthenticationPrincipal PersistentAuthorDetails authorDetails) {
        return fileService.findFilesHandler(authorDetails.getUserId(), authorDetails.getRoleName());
    }

    @RequestMapping(value = "/delete", method = RequestMethod.DELETE)
    public int deleteFileById(@RequestParam Map<String, String> allRequestParams, @AuthenticationPrincipal PersistentAuthorDetails authorDetails) {
    ...
    public ResponseEntity<Resource> downloadFile(@RequestParam Map<String, String> allRequestParams,
                                                 @AuthenticationPrincipal PersistentAuthorDetails authorDetails) {
        String uploadBy = Constants.ROLE_AUTHOR;
    ...
    public List<FileBean> getFileReports(@RequestParam("fileId") String fileId, @AuthenticationPrincipal PersistentAuthorDetails authorDetails) {
        return fileService.findReportsForAuthorFile(authorDetails.getUserId(), Long.parseLong(fileId));
    }

    @RequestMapping(value = "/report", method = RequestMethod.GET)
    public FileBean getFileReport(@RequestParam Map<String, String> allRequestParams, @AuthenticationPrincipal PersistentAuthorDetails authorDetails) {
